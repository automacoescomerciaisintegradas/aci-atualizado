...
export const MultiChannelPublisher: React.FC<{ onNavigate: (page: Page, context?: { from: Page }) => void; onAddCreditsClick: () => void; }> = ({ onNavigate, onAddCreditsClick }) => {
    ...
    // Publish Step State
    const [channels, setChannels] = useState({ instagram: true, telegram: true, blog: true });
    ...
    const handlePublishAll = useCallback(async () => {
        ...
        for (let i = 0; i < total; i++) {
            ...
            try {
                ...
                if (channels.blog) await new Promise(res => setTimeout(res, 200));
    ...
    }, [selectedProducts, generatedContents, channels, settings, isTelegramConfigured, isInstagramConfigured, saveSettings, onAddCreditsClick, isScheduled, scheduleDate, scheduleTime, scheduleMode, quantityPerDay, avoidHours, avoidStartTime, avoidEndTime]);
            ...
            <div className="bg-dark-card rounded-xl border border-dark-border p-6 md:p-8 animate-fade-in">
                <h2 className="text-xl font-semibold mb-1 text-dark-text-primary flex items-center gap-2">
                 ...
                 <div className="ml-10 mt-6 space-y-4">
                    ...
                    <ChannelSelector icon={<FileTextIcon/>} title="Blog Shopee" isConfigured={true} isSelected={channels.blog} onToggle={() => setChannels(p => ({...p, blog: !p.blog}))} />
